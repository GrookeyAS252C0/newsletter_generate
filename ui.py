        ...
        if use_google_calendar:
            ...
            try:
                ...
                if not os.path.exists('credentials.json'):
                    st.sidebar.error("❌ credentials.json が見つかりません")
                    st.sidebar.markdown("""
                    **設定方法:**
                    1. Google Cloud Consoleでプロジェクトを作成
                    2. Calendar APIを有効化
                    3. OAuth2認証情報を作成
                    4. credentials.jsonをダウンロードしてプロジェクトルートに配置
                    """)
                    return calendar_config
                
                # セッション状態でカレンダーリストをキャッシュ
                if 'available_calendars' not in st.session_state:
                    st.sidebar.info("📅 利用可能なカレンダーを取得中...")
                    try:
                        from calendar_service import GoogleCalendarService
                        temp_service = GoogleCalendarService()
                        st.session_state.available_calendars = temp_service.get_calendar_list()
                        st.sidebar.success("✅ カレンダーリスト取得完了")
                    except Exception as e:
                        st.sidebar.error(f"カレンダー取得エラー: {e}")
                        st.session_state.available_calendars = []
                
                available_calendars = st.session_state.available_calendars
                
                if available_calendars:
                    st.sidebar.success(f"✅ {len(available_calendars)} 個のカレンダーが利用可能")
                    
                    # カレンダー選択用のオプションを作成
                    calendar_options = {}
                    for cal in available_calendars:
                        primary_mark = " (メイン)" if cal.get('primary', False) else ""
                        display_name = f"{cal['summary']}{primary_mark}"
                        calendar_options[display_name] = cal['id']
                    
                    # 行事予定用カレンダーの選択
                    st.sidebar.markdown("**📅 行事予定用カレンダー**")
                    st.sidebar.caption("→ メルマガ「2. 今日の日大一」セクション")
                    selected_schedule_calendars = st.sidebar.multiselect(
                        "日常の行事予定を取得するカレンダーを選択",
                        options=list(calendar_options.keys()),
                        help="授業、会議、学校行事などの日常的なイベントを取得するカレンダー"
                    )
                    
                    # 広報イベント用カレンダーの選択
                    st.sidebar.markdown("**🎉 広報イベント用カレンダー**")
                    st.sidebar.caption("→ メルマガ「3. 今後の学校説明会について」セクション")
                    selected_event_calendars = st.sidebar.multiselect(
                        "学校説明会等のイベントを取得するカレンダーを選択",
                        options=list(calendar_options.keys()),
                        help="学校説明会、オープンキャンパス、入試説明会などの広報イベントを取得するカレンダー"
                    )
                    
                    # 選択されたカレンダーのIDを取得
                    calendar_config['schedule_calendar_ids'] = [
                        calendar_options[name] for name in selected_schedule_calendars
                    ]
                    calendar_config['event_calendar_ids'] = [
                        calendar_options[name] for name in selected_event_calendars
                    ]
                    
                    # 選択状況の表示とデバッグ情報
                    if calendar_config['schedule_calendar_ids']:
                        st.sidebar.success(f"📅 行事予定: {len(calendar_config['schedule_calendar_ids'])} カレンダー選択済み")
                        # デバッグ情報
                        with st.sidebar.expander("🔍 選択された行事予定カレンダー", expanded=False):
                            for i, cal_id in enumerate(calendar_config['schedule_calendar_ids'], 1):
                                st.sidebar.text(f"{i}. {cal_id}")
                    else:
                        st.sidebar.info("⚠️ 行事予定用カレンダーが未選択")
                    
                    if calendar_config['event_calendar_ids']:
                        st.sidebar.success(f"🎉 広報イベント: {len(calendar_config['event_calendar_ids'])} カレンダー選択済み")
                        # デバッグ情報
                        with st.sidebar.expander("🔍 選択された広報イベントカレンダー", expanded=False):
                            for i, cal_id in enumerate(calendar_config['event_calendar_ids'], 1):
                                st.sidebar.text(f"{i}. {cal_id}")
                    else:
                        st.sidebar.info("⚠️ 広報イベント用カレンダーが未選択")
                    
                    # イベントキーワードの設定
                    st.sidebar.markdown("**🔍 イベント検索キーワード**")
                    custom_keywords = st.sidebar.text_area(
                        "検索キーワード（1行1キーワード）",
                        value="\n".join(calendar_config['event_keywords']),
                        help="広報イベントを検索する際のキーワードを設定"
                    )
                    
                    if custom_keywords.strip():
                        calendar_config['event_keywords'] = [
                            keyword.strip() for keyword in custom_keywords.split('\n') 
                            if keyword.strip()
                        ]
                    
                    # カレンダー設定のリフレッシュボタン
                    if st.sidebar.button("🔄 カレンダーリストを更新", help="利用可能なカレンダーリストを再取得"):
                        if 'available_calendars' in st.session_state:
                            del st.session_state.available_calendars
                        st.rerun()
                        
                else:
                    st.sidebar.warning("⚠️ 利用可能なカレンダーが見つかりません")
                    
            except Exception as e:
                st.sidebar.error(f"❌ Google Calendar設定エラー: {e}")
