...
"""
    ...
    def _build_enhanced_message_generation_prompt(self, weather_info: WeatherInfo, 
        ...
        return f"""
以下のJSON天気データと最新の医学的知見をもとに、メールマガジンを読んでくれている受験生・保護者の体調を気遣う温かい丁寧語のメッセージを生成してください。
...
**重要な要求事項：**
1. **科学的根拠の活用**: 上記の医学的知見を自然に織り込む
2. **バリエーション重視**: 毎回異なる視点から体調を気遣う
3. **具体的アドバイス**: 耳マッサージ、水分補給、マッサージなど実践的提案
4. **全要素の統合**: 快適具合・月齢・気圧状況・降水確率すべてを含める
5. **文体**: 70-100文字程度の丁寧語、学校らしい品格

**文章の多様性を生むアプローチ**:
- 今日は「予防的観点」、「体のケア」、「心のケア」、「具体的対策」のいずれかの視点を選択
- 月齢と気圧の組み合わせから、毎回異なる表現パターンを使用
- 東洋医学的視点と現代医学的視点を交互に活用

必ず医学的根拠に基づいた体調管理アドバイスを含め、毎回異なる視点からのバリエーション豊かなメッセージを作成してください。
"""
