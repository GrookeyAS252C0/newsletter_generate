    ...
    def generate_heartwarming_message(self, weather_info: WeatherInfo, target_date: date) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "学校の入試広報部として、保護者や受験生に向けて体調を気遣う温かいメッセージを書く専門家です。抽出されたJSON天気データから、快適具合・月齢・気圧状況を基に、このメールマガジンを読んでくれている受験生・保護者の体調を気遣う文面を作成してください。降水確率が高い場合には雨具の心配もしてください。特に気温が高い日は、気圧が安定していても「心地よい」「快適」などの快適性を示す表現は避け、現実的な体感に合った表現を使ってください。すべての要素を自然に統合した心温まるメッセージにまとめてください。"},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=250,
                temperature=0.3
...
"""
    ...
    def _build_message_generation_prompt(self, weather_info: WeatherInfo, 
        ...
        return f"""
以下のJSON天気データをもとに、メールマガジンを読んでくれている受験生・保護者の体調を気遣う温かい丁寧語のメッセージを生成してください。
...
**気温と気圧の整合性ガイダンス**:
- 快適具合が「とても暑い」「暑い」の場合：
  - 高気圧 → 「安定した晴天」「強い日差し」「晴天続き」「日差しが強く」
  - 低気圧 → 「蒸し暑さ」「重い空気」「湿った空気」
- 快適具合が「過ごしやすい」「肌寒い」「涼しい」「寒い」の場合：
  - 高気圧 → 「心地よい気候」「爽やかな天気」「穏やかな晴天」
  - 低気圧 → 「穏やかな曇り空」「しっとりとした空気」

**重要な要求事項：**
1. **必須要素の組み込み**: 以下の全要素を必ずメッセージに反映させる
   - 快適具合（{weather_info.快適具合}）による体調への配慮
   - 月齢（{weather_info.月齢}）による体調への影響配慮
   - 気圧状況（{weather_info.気圧状況}）による体調への配慮
   - 降水確率が高い場合（50%以上）は雨具の心配

2. **具体的な組み込み方法**:
   - 快適具合から受験生・保護者の体調を気遣う表現
   - 月齢による体調変化への配慮を優しく表現
   - 気圧による体調への影響を気遣う表現
   - 降水確率が高い場合は雨具の準備への心配を含める
   - 全要素を調和させた体調気遣いのメッセージにする
...
4. **禁止事項**:
   ...
   - 快適具合・月齢・気圧状況・降水確率のいずれかを無視したメッセージは作らない
   - 情報に矛盾する内容は書かない
   - 「とても暑い」「暑い」時に「心地よい」「快適」「爽やか」などの快適性表現は使わない
   - 気温と気圧状況で矛盾する表現を避ける（例：猛暑日に「心地よい気候」など）

**例文参考**:
- 快適な気温の場合：「本日は{weather_info.快適具合}で過ごしやすい一日です。{weather_info.月齢}の時期は体調の変化を感じやすく、{weather_info.気圧状況}の影響もございますので、受験生の皆様も保護者の方々も、どうぞ体調管理にお気をつけください。」
- 暑い気温の場合：「本日は{weather_info.快適具合}一日となりますが、{weather_info.気圧状況}により安定した晴天です。{weather_info.月齢}の時期は体調の変化を感じやすいため、受験生の皆様も保護者の方々も、水分補給と体調管理にお気をつけください。」

必ず快適具合・月齢・気圧状況から体調を気遣い、降水確率が高い場合は雨具の心配も含めたメッセージを作成してください。
"""
