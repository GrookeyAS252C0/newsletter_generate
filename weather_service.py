    ...
    def generate_heartwarming_message(self, weather_info: WeatherInfo, target_date: date) -> str:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "学校の入試広報部として、保護者や受験生に向けて体調を気遣う温かいメッセージを書く専門家です。抽出されたJSON天気データから、快適具合・月齢・気圧状況を基に、このメールマガジンを読んでくれている受験生・保護者の体調を気遣う文面を作成してください。降水確率が高い場合には雨具の心配もしてください。すべての要素を自然に統合した心温まるメッセージにまとめてください。"},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=250,
                temperature=0.3
...
**例文参考**:
「本日は{weather_info.快適具合}で過ごしやすい一日です。{weather_info.月齢}の時期は体調の変化を感じやすく、{weather_info.気圧状況}の影響もございますので、受験生の皆様も保護者の方々も、どうぞ体調管理にお気をつけください。」
